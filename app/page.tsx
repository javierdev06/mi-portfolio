        ...
        {started && (
          <button
            onClick={() => {
              ...
              if (!musicGain) return
              if (!muted) musicGain.gain.setTargetAtTime(0, audioCtxRef.current!.currentTime, 0.3)
              else musicGain.gain.setTargetAtTime(0.04, audioCtxRef.current!.currentTime, 0.3)
              setMuted(m => !m)
            }}
            style={{
              position: "absolute",
